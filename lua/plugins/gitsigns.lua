...
return {
  {
    ...
    opts = {
      on_attach = function(bufnr)
        ...
        map('n', '<leader>gcb', gitsigns.toggle_current_line_blame, { desc = 'Toggle git show [c]urrent [b]lame line' })
        map('n', '<leader>gd', gitsigns.diffthis, { desc = 'git [d]iff against index' })
        map('n', '<leader>gD', function() gitsigns.diffthis '@' end, { desc = 'git [D]iff against last commit' })
        map('n', '<leader>ggD', gitsigns.preview_hunk_inline, { desc = 'Toggle [g]it show [D]eleted' })
